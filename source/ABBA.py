# Modified from https://github.com/nla-group/ABBA
...
class ABBA(object):
    ...
    def digitize(self, pieces):
        ...
        # num_k = np.unique(np.array(pieces)[:,:2], axis=1).shape[0]
        # if num_k >= self.min_k:
        #     self.min_k = num_k - 1
        #     if num_k >= self.max_k:
        #         self.max_k = num_k - 1
            ...
            # Use Kmeans
                ...
                if self.digitization_tol != 0:
                    ...
                    while k <= self.max_k-1 and (error > bound):
                        ...
                        if self.seed:
                            kmeans = KMeans(n_clusters=k, tol=0, n_init="auto", random_state=0).fit(data[:,1].reshape(-1,1))
                        else:
                            kmeans = KMeans(n_clusters=k, n_init="auto", tol=0).fit(data[:,1].reshape(-1,1))
                ...
                # Zero error so cluster with largest possible k.
                    ...
                    # tol=0 ensures labels and centres coincide
                    kmeans = KMeans(n_clusters=k, tol=0, n_init="auto").fit(data[:,1].reshape(-1,1))
                    centers = kmeans.cluster_centers_
                    labels = kmeans.labels_
            ...
            # Use Kmeans
                ...
                if self.digitization_tol != 0:
                    ...
                    while k <= self.max_k-1 and (error > bound):
                        ...
                        # tol=0 ensures labels and centres coincide
                        if self.seed:
                            kmeans = KMeans(n_clusters=k, tol=0, n_init="auto", random_state=0).fit(data[:,0].reshape(-1,1))
                        else:
                            kmeans = KMeans(n_clusters=k, n_init="auto", tol=0).fit(data[:,0].reshape(-1,1))
                        centers = kmeans.cluster_centers_
                        labels = kmeans.labels_
    ...
    def digitize_kmeans(self, data):
        ...
        if self.digitization_tol != 0:
            ...
            while k <= self.max_k-1 and (error > bound):
                ...
                # tol=0 ensures labels and centres coincide
                if self.seed:
                    kmeans = KMeans(n_clusters=k, n_init="auto", tol=0, random_state=0).fit(data)
                else:
                    kmeans = KMeans(n_clusters=k, n_init="auto", tol=0).fit(data)
                centers = kmeans.cluster_centers_
                labels = kmeans.labels_
